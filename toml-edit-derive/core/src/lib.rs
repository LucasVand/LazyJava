...
pub fn inline_item(entries: impl IntoIterator<Item = (String, Item)>) -> Item {
    ...
    for (k, item) in entries {
        match item {
            ...
            _ => panic!(
                "toml_edit_derive::inline_item: only Value items are supported in inline tables"
            ),
        }
    }
...
impl TomlEditValue for PathBuf {
    ...
    fn into_value(self) -> Value {
        Value::from(self.to_string_lossy().into_owned())
    }
}
...
impl<'a, T: TomlEditable> FieldEntry<'a, T> {
    ...
    pub fn is_some(&self) -> bool {
        self.table.contains_key(self.key)
    }
    pub fn is_none(&self) -> bool {
        !self.is_some()
    }
    pub fn get(&self) -> Option<T::View<'_>> {
        self.table.get(self.key).and_then(T::from_item)
    }
    ...
    pub fn set(&mut self, v: T) {
        ...
        match self.table.get_mut(self.key) {
            Some(slot) => {
                transfer_value_decor(slot, &mut item);
                *slot = item;
            }
            None => {
    ...
    pub fn get_or_insert(self, default: T) -> T::Mut<'a> {
        self.insert_if_absent(T::into_item(default))
    }
    pub fn get_or_insert_with(self, f: impl FnOnce() -> T) -> T::Mut<'a> {
        self.insert_if_absent(T::into_item(f()))
    }
    pub fn get_or_insert_empty(self) -> T::Mut<'a> {
        self.insert_if_absent(T::empty_item())
    }
    pub fn get_or_insert_default(self) -> T::Mut<'a>
    where
        T: Default,
    {
        self.insert_if_absent(T::into_item(T::default()))
    }
    fn get_mut_present(self) -> T::Mut<'a> {
        self.table
            .get_mut(self.key)
            .and_then(T::from_item_mut)
            .expect("entry missing after insert")
    }
    #[track_caller]
    ...
    pub fn remove(&mut self) -> bool {
        self.table.remove(self.key).is_some()
    }
    ...
    pub fn and_mutate<F>(mut self, f: F) -> bool
    where
        F: FnOnce(&mut T::Mut<'_>),
    {
        if let Some(mut m) = self.get_mut() {
            f(&mut m);
            true
        } else {
            false
...
impl<'a, T: TomlEditValue> TomlEditValueMut<'a, T> {
    pub fn new(item: &'a mut Item) -> Self {
        Self {
            inner: ValueOrItem::Item(item),
            _marker: PhantomData,
        }
    }
    pub(crate) fn from_value(v: &'a mut Value) -> Self {
        Self {
            inner: ValueOrItem::Value(v),
            _marker: PhantomData,
        }
    }
    ...
    pub fn set(&mut self, v: T) {
        ...
        match &mut self.inner {
            ValueOrItem::Value(slot) => {
                *new_val.decor_mut() = slot.decor().clone();
                **slot = new_val;
            }
            ValueOrItem::Item(slot) => {
                if let Item::Value(ref old_val) = **slot {
                    *new_val.decor_mut() = old_val.decor().clone();
                }
...
impl<T: TomlEditValue> TomlEditable for T {
    ...
    fn from_value(v: &Value) -> Option<T> {
        <T as TomlEditValue>::from_value(v)
    }
    fn from_item(item: &Item) -> Option<T> {
        item.as_value()
            .and_then(|v| <T as TomlEditValue>::from_value(v))
    }
    fn from_item_mut(item: &mut Item) -> Option<TomlEditValueMut<'_, T>> {
        item.is_value().then(|| TomlEditValueMut::new(item))
    }
    fn from_value_mut(v: &mut Value) -> Option<TomlEditValueMut<'_, T>> {
        Some(TomlEditValueMut::from_value(v))
    }
    fn into_item(self) -> Item {
        Item::Value(T::into_value(self))
    }
}
...
pub trait TomlEditable: Sized {
    ...
    fn from_item(item: &Item) -> Option<Self::View<'_>> {
        item.as_table_like().and_then(Self::from_table_like)
    }
    fn from_table_like(table: &dyn TableLike) -> Option<Self::View<'_>> {
        let _ = table;
        None
    }
    fn from_value(_v: &Value) -> Option<Self::View<'_>> {
        None
    }
    fn from_item_mut(item: &mut Item) -> Option<Self::Mut<'_>> {
        item.as_table_like_mut().and_then(Self::from_table_like_mut)
    }
    fn from_table_like_mut(table: &mut dyn TableLike) -> Option<Self::Mut<'_>> {
        let _ = table;
        None
    }
    fn from_value_mut(_v: &mut Value) -> Option<Self::Mut<'_>> {
        None
    }
    fn into_item(self) -> Item;
    fn empty_item() -> Item {
        ...
        impl<T: TomlEditable + 'static> TomlEditable for std::collections::$map<String, T> {
            ...
            fn from_table_like(table: &dyn TableLike) -> Option<Self::View<'_>> {
                Some(
                    table
                        .iter()
                        .filter_map(|(k, i)| Some((k.to_owned(), T::from_item(i)?)))
                        .collect(),
                )
            }
            fn from_item_mut(item: &mut Item) -> Option<TomlEditMapMut<'_, T>> {
                item.as_table_like_mut().map(TomlEditMapMut::new)
            }
            fn into_item(self) -> Item {
                let mut __tbl = Table::new();
                for (k, v) in self {
                    __tbl.insert(&k, T::into_item(v));
                }
                Item::Table(__tbl)
            }
        }
...
impl<T: TomlEditable + 'static> TomlEditable for Vec<T> {
    ...
    fn from_item(item: &Item) -> Option<Vec<T::View<'_>>> {
        if let Some(arr) = item.as_array() {
            Some(arr.iter().filter_map(T::from_value).collect())
        } else {
            item.as_array_of_tables()
                .map(|aot| aot.iter().filter_map(|t| T::from_table_like(t)).collect())
        }
    }
    ...
    fn into_item(self) -> Item {
        ...
        assert!(
            !(saw_values && saw_tables),
            "Vec::into_item: mixed Value/Table"
        );
        assert!(
            !(T::IS_VALUE_TYPE && saw_tables),
            "Vec::into_item: IS_VALUE_TYPE mismatch"
        );
        if T::IS_VALUE_TYPE || saw_values {
...
where
    ...
    R: std::iter::FusedIterator<Item = L::Item>,
{
}
...
impl<'a, T: TomlEditable> TomlEditArrayMut<'a, T> {
    pub fn new(arr: &'a mut ArrayOfTables) -> Self {
        Self {
            storage: ArrayMutStorage::Tables(arr),
            _marker: PhantomData,
        }
    }
    fn new_values(arr: &'a mut Array) -> Self {
        Self {
            storage: ArrayMutStorage::Values(arr),
            _marker: PhantomData,
        }
    }
    pub fn len(&self) -> usize {
        match &self.storage {
            ArrayMutStorage::Values(a) => a.len(),
            ArrayMutStorage::Tables(a) => a.len(),
        }
    }
    ...
    pub fn iter(&self) -> impl Iterator<Item = T::View<'_>> + '_ {
        match &self.storage {
            ArrayMutStorage::Values(a) => {
                EitherIter::Left(a.iter().filter_map(|v| T::from_value(v)))
            }
            ArrayMutStorage::Tables(a) => {
                EitherIter::Right(a.iter().filter_map(|t| T::from_table_like(t)))
            }
        }
    }
    pub fn iter_mut(&mut self) -> impl Iterator<Item = T::Mut<'_>> + '_ {
        match &mut self.storage {
            ArrayMutStorage::Values(a) => {
                EitherIter::Left(a.iter_mut().filter_map(|v| T::from_value_mut(v)))
            }
            ArrayMutStorage::Tables(a) => {
                EitherIter::Right(a.iter_mut().filter_map(|t| T::from_table_like_mut(t)))
            }
        }
    }
    #[must_use]
    pub fn push_empty(&mut self) -> T::Mut<'_> {
        match &mut self.storage {
            ...
            ArrayMutStorage::Tables(a) => {
                a.push(Table::new());
                let last = a.len() - 1;
                T::from_table_like_mut(a.get_mut(last).unwrap())
                    .expect("push_empty requires struct type")
            }
        }
    }
    ...
    pub fn at(&mut self, index: usize) -> Option<T::Mut<'_>> {
        self.get_mut(index)
    }
    pub fn clear(&mut self) {
        match &mut self.storage {
            ArrayMutStorage::Values(a) => a.clear(),
            ArrayMutStorage::Tables(a) => a.clear(),
        }
    }
}
impl<T: TomlEditable> std::fmt::Debug for TomlEditArrayMut<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TomlEditArrayMut").finish()
    }
}
...
impl<'a, T: TomlEditable> TomlEditMapMut<'a, T> {
    pub fn new(table: &'a mut dyn TableLike) -> Self {
        Self {
            table,
            _marker: PhantomData,
        }
    ...
    pub fn get(&self, key: &str) -> Option<T::View<'_>> {
        self.table.get(key).and_then(T::from_item)
    }
    pub fn get_mut(&mut self, key: &str) -> Option<T::Mut<'_>> {
        self.table.get_mut(key).and_then(T::from_item_mut)
    }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (&str, T::View<'_>)> + '_ {
        self.table
            .iter()
            .filter_map(|(k, i)| Some((k, T::from_item(i)?)))
    }
    pub fn iter_mut(&mut self) -> impl Iterator<Item = (String, T::Mut<'_>)> + '_ {
        self.table
            .iter_mut()
            .filter_map(|(k, i)| Some((k.get().to_owned(), T::from_item_mut(i)?)))
    }
    #[must_use]
...
impl<T: TomlEditable> std::fmt::Debug for TomlEditMapMut<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TomlEditMapMut").finish()
    }
}
